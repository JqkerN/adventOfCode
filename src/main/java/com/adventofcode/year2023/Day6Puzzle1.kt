...
class Day6Puzzle1 {
    ...
    fun solvePuzzle(puzzleInput: List<String>): Int {
        var score = 1
        val times = numberRegex.findAll(puzzleInput[0]).map { it.value.toDouble() }.toList()
        val distances = numberRegex.findAll(puzzleInput[1]).map { it.value.toDouble() }.toList()
    ...
    fun pqFormula(p: Double, q: Double): Pair<Double, Double> {
        val pHalf = p / 2.0
