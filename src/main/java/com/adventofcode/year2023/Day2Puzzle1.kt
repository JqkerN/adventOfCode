...
fun main() {
    ...
    val puzzleSolution = Day2Puzzle1().solvePuzzle(problem)
    println("Puzzle 1 solution: %s".format(puzzleSolution))
}

class Day2Puzzle1 {
    private val gameIndexRegex = "\\AGame (\\d+):.*\$".toRegex()
    ...
    fun solvePuzzle(gameRecords: List<String>): Int {
        val indices = getIndexFromValidGameRecords(gameRecords)
        return indices.sum()
    }

    private fun getIndexFromValidGameRecords(
        gameRecords: List<String>,
    ): ArrayList<Int> {
        val validGameRecordsIndices = arrayListOf<Int>()
        for (gameRecord in gameRecords) {
            val gameIndex = getGameIndex(gameRecord)
            val gameOutcomes = getGameOutcomes(gameRecord)
            if (hasValidOutcomes(gameOutcomes)) {
                validGameRecordsIndices.add(gameIndex)
            }
        }
        return validGameRecordsIndices
    }

    private fun hasValidOutcomes(gameOutcomes: ArrayList<ArrayList<CubeOutcome>>): Boolean {
        for (gameOutcome in gameOutcomes) {
            for (cubeOutcomes in gameOutcome) {
                if (cubeOutcomes.amount > cubeOutcomes.cubeColorLimits[cubeOutcomes.cubeColor]!!) {
                    return false
                }
            }
        }
    ...
    private fun getGameIndex(gameRecord: String): Int {
        val match = gameIndexRegex.find(gameRecord)
    ...
    class CubeOutcome(var cubeColor: CubeColor, var amount: Int) {
        val cubeColorLimits = mapOf(
            Pair(CubeColor.RED, 12),
            Pair(CubeColor.GREEN, 13),
            Pair(CubeColor.BLUE, 14)
        )
    }
